...
$sql="SELECT COUNT(proceso)FROM flujousuario WHERE usuario like '".$_SESSION["ci"]."' AND fechafin is null";
...
if($nroproceso==0){
    ...
    $resultado=mysqli_query($conn, $sql);
    $fila=mysqli_fetch_array($resultado);
    $notramite=$fila["numerotramite"];

    #echo $notramite;
    
    $sql_p="SELECT * FROM flujo WHERE tipo like 'I'";
    echo $sql_p.'<br>';
    $resultado_p =mysqli_query($conn, $sql_p);
    $fila_p=mysqli_fetch_array($resultado_p);
    $proceso=$fila_p["proceso"];
    $flujo=$fila_p["flujo"];
    $sql="insert into flujousuario (numerotramite,usuario,flujo,proceso,fechainicio) values(".$notramite.",'".$_SESSION["ci"]."','".$flujo."','".$proceso."','".$Date."')";
