<?php   
	...
	$sql="select * from flujousuario where usuario='".$_SESSION["ci"]."' and fechafin is null";
	...
	$sql_sc="select * from inscripcion c, tramite t where t.ci='".$_SESSION["ci"]."' and t.numerotramite like c.numerotramite";
	...
	<div class ="contenedor">
			...
			<?php
				if(empty($fila)){
					$sql2="select * from flujousuario where usuario='".$_SESSION["ci"]."' ";
					...
					if(empty($fila2)){
						...
						if ($estado=='inscrito') {
							echo 'Tu solicitud fue realizada con exito, está inscrito';
						}else{
							if ($estado=='rechazado') {
			...
			<table class="tabla">
				<?php
					...
					while($fila)
					{
						...
						echo "<tr>";
						echo "<td> N° Trámite: ".$fila["numerotramite"]."<br>";
						echo "Flujo - Proceso: ".$fila["flujo"]." - ".$fila["proceso"]."<br>";
						echo "Fecha de Inicio: ".$fila["fechainicio"]."<br>";
						echo "Estado: Pendiente ";
				?>
					<td style='width:20%;'>
					<a class="boton3" href='desflujo.php?flujo=<?php echo $fila["flujo"]?>&proceso=<?php echo $fila["proceso"]?>&numerotramite=<?php echo $fila["numerotramite"]?>'>
						Mostrar </a> <br>		
				<?php		
						$fila=mysqli_fetch_array($resultado);
						}	
